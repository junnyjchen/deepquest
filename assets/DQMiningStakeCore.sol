...
contract DQMiningStakeCore is ReentrancyGuard {
    ...
    // 手续费地址 (0x18509333fDD732ae37FbcBf8F32F707A15f8FdE4)
    address constant FEE_RECEIVER = 0x18509333fDD732ae37FbcBf8F32F707A15f8FdE4;
    ...
    /**
     * @notice 用户移除LP质押
     * @dev 根据质押时间计算手续费：60天内20%，61-180天10%，180天后0%
    ...
    function claimLPReward() external nonReentrant {
        ...
        if (reward > 0) {
            ...
            // 转账
            if (fee > 0) {
                IERC20(dqToken).safeTransfer(feeReceiver, fee);
            }
