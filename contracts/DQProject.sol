 ...
 * - 兑换对: DQ ↔ USDT (使用 0x570A5D26f7765Ecb712C0924E4De545B89fD43dF 作为参考价格)
 * 
 * 质押周期与分红:
 * - 30天: 5%
 * - 90天: 10%  
...
contract DQProject is Ownable, ReentrancyGuard {
    ...
    // USDT 代币地址 (SOL 参考 - 实际使用 USDT)
    address public constant USDT_ADDRESS = 0x570A5D26f7765Ecb712C0924E4De545B89fD43dF;
    
    // 价格相关 (1 DQ = ? USDT, 默认 1:1，后续可通过预言机更新)
    uint256 public dqPrice = 1 ether;  // 1 DQ = 1 USDT
    ...
    uint256[] public stakeRates = [5, 10, 15, 20];   // 总和 50%
    ...
    function blockMining() external nonReentrant {
        ...
        // 分配: LP 60%, NFT 15%, 基金会 5%, 团队 14%, 合伙人 6%
     ...
     * @notice 质押 DQ 代币
     * @param _amount 质押数量
     * @param _periodIndex 周期索引 (0=30天, 1=90天, 2=180天, 3=360天)
     */
    ...
    function swapDQForUSDT(uint256 _dqAmount) external nonReentrant {
        ...
        require(IERC20(USDT_ADDRESS).balanceOf(address(this)) >= usdtAmount, "insufficient USDT in pool");
    ...
    function swapUSDTForDQ(uint256 _usdtAmount) external nonReentrant {
        ...
        require(dqToken.totalSupply() - dqToken.balanceOf(address(this)) + dqAmount <= 100_000_000_000 * 10**18, "exceed max supply");
    ...
    function getSwapQuote(uint256 _dqAmount) external view returns (uint256 usdtOut) {
        return _dqAmount * dqPrice / 1 ether;
    }

    function getReverseSwapQuote(uint256 _usdtAmount) external view returns (uint256 dqOut) {
        return _usdtAmount * 1 ether / dqPrice;
    }
