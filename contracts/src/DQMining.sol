...
// 说明：本文件逻辑严格对齐 assets/DQMining.sol。
// 仅将 import 路径改为 Hardhat 本地可编译形式，未改动业务逻辑。
...
contract DQMining is ReentrancyGuard {
    ...
    modifier onlyReg() {
        require(users[msg.sender].referrer != address(0) || msg.sender == OWNER, "!reg");
        _;
    }
    ...
    function importUsers(address[] calldata _u, address[] calldata _r) external onlyOwner {
        ...
        for (uint i = 0; i < _u.length; i++) {
            ...
            allUsers.push(_u[i]);
            if (_r[i] != address(0) && (users[_r[i]].referrer != address(0) || _r[i] == OWNER)) {
